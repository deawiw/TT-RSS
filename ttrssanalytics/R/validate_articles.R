...
#' @title Validate normalized articles
#' @description Validates normalized article rows for required columns, required values, datetime format, URL format, and duplicate keys.
#' @param articles_df Data frame of normalized articles.
#' @param transform_stats Optional transform statistics list used in the validation report.
#' @return Validation report list with ok flag, warnings, errors, counts, and transform stats.
#' @export
...
#' @title Assert validation success
#' @description Stops with a combined validation message if the validation report contains errors.
#' @param report Validation report returned by validate_articles_df().
